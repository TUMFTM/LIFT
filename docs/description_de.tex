...
Dafür muss es einerseits mit wenigen, schnell abzuschätzenden Eingangsparametern arbeiten und seine Ergebnisse schnell und intuitiv darstellen.
...
\label{sec:gui}
\ac{lift} ist in Python programmiert und verfügt über eine browserbasierte \ac{gui}, die auf der streamlit-Bibliothek basiert.
Die Installation von \ac{lift}, entweder auf einem lokalen Computer oder auf einem Server ist im \texttt{README} des dazugehörigen git repository beschrieben.
...
Berechnete Kennwerte werden jeweils vergleichend für Baseline- und Expansions-Szenario verglichen, um Effekte der getroffenen Maßnahmen intuitiv beurteilen zu können.
Für die angezeigten Ergebnisse und Grafiken sind ebenfalls erläuternde Hilfetexte vorhanden.

\section{Methodik}
\label{sec:methods}

Dieses Kapitel beschreibt die dem Tool \ac{lift} zugrundeliegende Methodik.
Wie bereits beschrieben, basiert \ac{lift} auf einigen Annahmen, die sich in methodische Annahmen und quantitative Annahmen unterteilen lassen.
...
Sie sind somit nur bei einer Benutzung von \ac{lift} durch die \ac{gui} zutreffend und separat in \autoref{sec:defaultvalues} aufgeführt.

\subsection{Energiesystem}
\label{subsec:energysystem}

Das von \ac{lift} abgebildete Energiesystem besteht aus verschiedenen Komponenten, von denen bis auf Ladeinfrastruktur und Fahrzeuge genau ein Element in definierbarer Größe vorhanden ist.
Im Energiesystem von \ac{lift} existieren folgende Komponententypen:
\begin{itemize}
    ...
    In \ac{lift} können verschiedene Typen von Ladeinfrastruktur modelliert werden.
...
Den Kern von \ac{lift} stellt eine Zeitschrittsimulation dar, die die Energieflüsse im Energiesystem und der Flotte des Depots simuliert.
