...
In diesem Energiesystemmodell werden zu jedem simulierten Zeitschritt Leistungsflüsse heuristisch (regelbasiert) bestimmt (\autoref{subsec:sim}), deren integrierte Energieflüsse die Basis für die Berechnung von \ac{opex} im Simulationszeitraum (\autoref{subsec:opex}) sind.
...
Für alle kontinuierlich dimensionierbaren Blöcke $b_\text{cont}$ (\ac{GCP}, \ac{PV} und \ac{ess}) werden diese jeweils in Abhängigkeit der gewählten Größe des Blocks $S^b$ sowie den spezifischen Werten pro Größeneinheit $c^b_{cap}$ und $e^b_{cap}$ berechnet.
...
Die hier jeweils für den gesamten Projektzeitraum berechneten Summen werden in \ac{lift} tatsächlich vektorisiert durchgeführt, sodass nominelle \ac{capex}, nominelle \ac{opex} und Diskontierungsfaktor für jedes Projektjahr verfügbar sind.
...
In \ac{lift} wird zwischen technischen und ökonomischen\,/\,ökologischen \acp{kpi} unterschieden.
...
Ökonomische\,/\,ökologische \ac{kpi} werden auf Basis der gemäß \autoref{subsec:extrapolation} vektorisierten (nach Projektjahren aufgelösten) diskontierten Gesamtkosten und -emissionen berechnet, die in der \ac{gui} als Cashflow-Diagramm dargestellt sind.

Die \textbf{Amortisationszeit} $t_\text{pb}$ gibt das Projektjahr an, an dessen Ende dem das Expansion-Szenario geringere diskontierte Gesamtkosten bzw. -emissionen aufweist.
Aufgrund der diskontinuierlich auftretenden \ac{capex} kann es Parameterkonfigurationen geben, die zu mehrfacher Amortisation führen.
Dies wird in $t_\text{pb}$ vernachlässigt und es wird nur das erste Amortisationsjahr ausgegeben.
