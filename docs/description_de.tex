...
\DeclareAcronym{ess}{short=ESS, long=stationärer Energiespeicher,}
...
\ac{capex} hängen unmittelbar von den Eingangsparametern ab und können daher unabhängig von Simulation (\textit{a priori}) berechnet werden (\autoref{subsec:capex}).

Zur Ermittlung von \ac{opex} modelliert und simuliert \ac{lift} je Szenario ein Energiesystem, das aus den Komponenten \ac{gcp}, \ac{pv}-Anlage, \ac{ess}, \ac{dem}, \ac{flt} und \ac{cis} besteht.
Jede dieser Komponenten existiert genau einfach im modellierten Energiesystem, kann aber durch entsprechende Parameterwertwahl (z.B. $P_{max}=0$) auch deaktiviert werden.
Flotte und \ac{cis} können variable Anzahlen an Subkomponenten (Ladepunkt-Leistungsklassen und Subflotten) enthalten.
Eine Subflotte fasst variable Anzahlen von Fahrzeugen des gleichen Typs und Nutzungsprofils zusammen.
In diesem Energiesystemmodell werden zu jedem simulierten Zeitschritt Leistungsflüsse bestimmt (\autoref{subsec:sim}), deren integrierte Energieflüsse die Basis für die Berechnung von \ac{opex} im Simulationszeitraum (\autoref{subsec:opex}) und technischen \acp{kpi} (\autoref{subsec:kpi_tech}) sind.

Gesamtkosten und -emissionen werden unter Annahme von Komponentenlebensdauern und entsprechenden Ersatzbeschaffungen sowie Kapitalkosten auf einen Projektzeitraum, der typischerweise deutlich länger als der Simulationszeitraum ist, extrapoliert (\autoref{subsec:extrapolation}) und zu ökonomischen bzw. ökologischen \acp{kpi} aggregiert (\autoref{subsec:kpi_econ}).
...
\subsection{Energiesystemkomponenten}
\label{subsec:system}

Der \textbf{\acf{gcp}} modelliert die Schnittstelle zum elektrischen Netz, unabhängig von dessen Spannungsebene, also die örtliche Messstelle.
Blindleistung im Wechselstromnetz wird nicht berücksichtigt.

Die \textbf{\ac{pv}-Anlage} modellieret ein homogen und unbeweglich für maximales Energiepotential ausgerichtetes und augeständertes Array von polykristallinen \ac{pv}-Modulen inklusive ihres Wechselrichters.
Das entsprechende zeitvariable Leistungspotential wird für den angegebenen Standort in stündlicher Auflösung vom \ac{pvgis} der Europäischen Union für das Jahr 2023 abgerufen.
Neuere oder zeitlich feiner aufgelöste Daten sind bei diesem Anbieter nicht verfügbar.
Während des Betriebs entstehen typischerweise weder Kosten noch Emissionen. \todo{verifizieren, dass der Code das theoretisch kann - brauchts diesen Satz überhaupt oder ist das nicht inhärent in Standardwerten}

Der \textbf{\acf{dem}} modelliert den Energieverbrauch der Gebäude und sonstiger Infrastruktur am Standort exklusive möglicherweise vorhandener Ladepunkte.
...
\subsection{\ac{capex}-Berechnung}
\label{subsec:capex}

Beim Aufbau einer Komponente $k$ (mit Ausnahme des Standortverbrauchs, der Ladeinfrastruktur und der Flotte) entstehen ökonomische (\autoref{eq:capex_init_econ}) und ökologische (\autoref{eq:capex_init_ecol}) \ac{capex}, die jeweils in Abhängigkeit der Dimensionierung der Komponente $S^k$ sowie den spezifischen Werten $c^k_{cap}$ und $e^k_{cap}$ pro Größeneinheit der Komponente berechnet und über alle Komponenten addiert werden.
Für Ladeinfrastruktur und Flotte ist keine Dimensionierungsabhängigkeit vorgesehen, sondern es entstehen fixe \ac{capex} pro Element $n$ (Fahrzeug oder Ladepunkt) einer Subkomponente bzw. -klasse $m$ (Fahrzeug- oder Ladepunkttyp).
...
\subsection{Zeitschrittsimulation}
\label{subsec:sim}
...
Trotz des szenarioadpativen Vorgehens kann zu kleine Dimensionierung der drei Quellenkomponenten in unerfüllbaren Anforderungen resultieren und eine entsprechende Fehlermeldung (``Netzanschlussfehler'') hervorrufen.
Wirkungsgrade, dynamische Strompreise und Flexibilitäten werden nicht berücksichtigt. \todo{Aussage prüfen}
...
\subsection{Technische \acfp{kpi}}
