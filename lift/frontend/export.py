...
def create_report(plots, inputs: pd.DataFrame):
    ...
    heading = doc.add_heading(get_label("report.title"), level=0)
    ...
    cell_paragraph.text = get_label("main.kpi_diagrams.costs.title")
    cell_paragraph, _ = format_cell(hdr_cells[1])
    cell_paragraph.text = get_label("main.kpi_diagrams.emissions.title")
    ...
    cell_paragraph, _ = format_cell(hdr_cells[0])
    cell_paragraph.text = get_label("main.kpi_diagrams.self_sufficiency.title")
    cell_paragraph, _ = format_cell(hdr_cells[1])
    cell_paragraph.text = get_label("main.kpi_diagrams.self_consumption.title")
    cell_paragraph, _ = format_cell(hdr_cells[2])
    cell_paragraph.text = get_label("main.kpi_diagrams.home_charging.title")
    ...
    time_plots = plots["time_diagrams"]
    doc.add_heading(get_label("main.time_diagrams.costs.title.label"), level=1)
    paragraph = doc.add_paragraph("")
    run = paragraph.add_run()
    run.add_break(WD_BREAK.LINE)
    ...
    doc.add_heading(get_label("main.time_diagrams.emissions.title.label"), level=1)
    paragraph = doc.add_paragraph("")
    run = paragraph.add_run()
    run.add_break(WD_BREAK.LINE)
    ...
    for row_idx, (index, row) in enumerate(inputs.iterrows()):
