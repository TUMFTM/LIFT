...
@safe_cache_data
def get_log_dem(slp: str, consumption_yrl_wh: float, settings: SimInputSettings) -> np.typing.NDArray[np.float64]:
    return (
        pd.concat(
            [
...
@safe_cache_data
def get_log_subfleet(vehicle_type: str, settings: SimInputSettings) -> pd.DataFrame:
...
def simulate(
    ...
    pv = PVSource(
        pwr_wp=capacities.pv_wp,
        log_spec=logs.pv_spec,
        dti=dti,
        freq_hours=freq_hours,
    )
    ess = StationaryStorage(
        capacity_wh=capacities.ess_wh,
        dti=dti,
        freq_hours=freq_hours,
    )
    grid = GridConnection(
        pwr_max_w=capacities.grid_w,
        dti=dti,
        freq_hours=freq_hours,
    )
...
def calc_phase_results(
    ...
    for sf_in in subfleets.values():
        ...
        cashflow[CONV["vehicles"]]["capex"] += (
            num_bev * sf_in.capex_bev_eur * replacements + num_icev * sf_in.capex_icev_eur * replacements
        )
        emissions[CONV["vehicles"]]["capex"] += (
            num_bev * sf_in.capem_bev * replacements + num_icev * sf_in.capem_icev * replacements
        )

        # bev
        dist_bev = log.loc[:, pd.IndexSlice[bevs, "dist"]].to_numpy().sum() if bevs else 0
        cashflow[CONV["vehicles"]]["opex"][:period_eco] += (
            num_bev * economics.insurance_frac * sf_in.capex_bev_eur  # insurance
            + dist_bev
            * (
        ...
        cashflow[CONV["vehicles"]]["opex"][:period_eco] += (
            num_icev * economics.insurance_frac * sf_in.capex_icev_eur  # insurance
            + dist_icev
            * (
        ...
        emissions[CONV["vehicles"]]["opex"][:period_eco] += (
            dist_icev * sf_in.consumption_icev / 100 * economics.co2_per_liter_diesel_kg
        )
...
def run_backend(inputs: Inputs) -> TotalResults:
    ...
    logs = Logs(
        pv_spec=get_log_pv(coordinates=inputs.location.coordinates, settings=inputs.economics.get_sim_input()),
        dem=get_log_dem(
            slp=inputs.location.slp.lower(),
        ...
        fleet={
            vehicle_type: get_log_subfleet(vehicle_type=vehicle_type, settings=inputs.economics.get_sim_input())
    ...
    results = {
        phase: calc_phase_results(
            ...
            subfleets={sf.name: sf.get_phase_input(phase=phase) for sf in inputs.subfleets.values()},
            chargers={chg.name: chg.get_phase_input(phase=phase) for chg in inputs.chargers.values()},
        )
