...
class SettingsInput(ABC):
    @abstractmethod
    def get_input(self, label: str, key: Optional[str] = None, domain=st): ...
...
class InputInvestComponent(PhaseInvestComponent):
    ...
    def get_input_component(self, phase: str) -> PhaseInvestComponent:
        return PhaseInvestComponent(
            capacity=self.capacity.get_value(phase=phase),
...
class PhaseInputLocation:
    ...
    def get_sim_input(self) -> SimInputLocation:
        return SimInputLocation(
            grid_w=self.grid.capacity,
...
class InputLocation(PhaseInputLocation):
    ...
    def get_phase_input(self, phase: str) -> PhaseInputLocation:
        return PhaseInputLocation(
            grid=self.grid.get_input_component(phase=phase),
...
class InputSubfleet(PhaseInputSubfleet):
    ...
    def get_phase_input(self, phase: Literal["baseline", "expansion"]) -> PhaseInputSubfleet:
        return PhaseInputSubfleet(
            name=self.name,
...
class PhaseInputCharger(SimInputCharger):
    ...
    def get_sim_input(self) -> SimInputCharger:
        return SimInputCharger(name=self.name, num=self.num, pwr_max_w=self.pwr_max_w)
...
class InputCharger(PhaseInputCharger):
    ...
    def get_phase_input(self, phase: Literal["baseline", "expansion"]) -> PhaseInputCharger:
        return PhaseInputCharger(
            name=self.name,
...
class InputEconomics(PhaseInputEconomics):
    def get_phase_input(self, phase: Literal["baseline", "expansion"]) -> "PhaseInputEconomics":
...
class Inputs:
    ...
    subfleets: dict[str, InputSubfleet] = field(
        default_factory=lambda: dict(
            hlt=InputSubfleet(name="hlt"),
            hst=InputSubfleet(name="hst"),
        )
    )
    chargers: dict[str, InputCharger] = field(
        default_factory=lambda: dict(
            ac=InputCharger(name="ac"),
